...
public class MigrationJobController : ControllerBase
{
    ...
    public async Task<IActionResult> ApproveJob(int id, [FromBody] ApproveRequest request)
    {
        ...
        try
        {
            ...
            // Check Replace Applied (Part 2 v5.2+) logs this when a planned edit did not
            // match the file. Such an edit is neither applied nor deferred - the reviewer
            // has to know it exists, or the PR looks more complete than it is.
            var failedEdits = await _context.JobLogs.CountAsync(l =>
                l.MigrationJobId == job.Id && l.Message.StartsWith("A planned file edit did not apply"));
            ...
            var bodyLines = new List<string>
            {
                ...
                deferred.Count == 0
                    ...
                    : $"- Only the changes required to build and run on `{tfm}` were applied. **{deferred.Count} were deferred** — see below.",
            };
            if (failedEdits > 0)
                bodyLines.Add($"- ⚠️ **{failedEdits} planned edit(s) did not apply** (the target text was not found). See the job log before merging.");
