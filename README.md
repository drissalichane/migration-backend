...
Use **Login with GitHub**, not the username/password form — pull requests need your GitHub
account. Every GitHub account that signs in gets the Admin role.
...
1. **Fork it** to your own GitHub account (the **Fork** button on its GitHub page). The app has to
   be able to push a branch to the repository it migrates, so it must be yours.
...
| A job goes to **Failed** a few minutes into *Analyzing* | Open n8n (http://localhost:5678) → **Executions**, click the failed run: the red node says why. The most common cause is the OpenRouter key — wrong, or out of credit. Fix `.env`, run `docker compose up -d`, start a new migration. |
